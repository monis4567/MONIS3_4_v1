...
# add 1 to a an increasing list of numbers
i <- 1
#make an empty list that can be filled with plots
lplotno <- list()
...
for (spec.lat in latspecnm){
  ...
  pdf(c(paste(wd00_10,"/","suppmatr_10.04b_App_A",no.spc.app.plot,"_stddilser_",sbs.abbr.nm,".pdf",  sep = ""))
  ...
  # #record the plot 
  plot01 <- recordPlot()
  #paste a string together to get a variable
  pltno <- paste("plotno_",i,sep="")
  #assign the plot to the new variable
  assign(pltno,plot01)
