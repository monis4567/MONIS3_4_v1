  ...
  for (yr_smpl in yrs){
    ...
    for (season in rctse){
        ...
        #start curly bracket for 'else if' test testing whether the dimensions on the data frame is more than one, if it is then make the plot on the map
      {
        ...
        #head(df_sbs.MO4_03,4)
        #make points with decimal coordinates - this makes use of a different CRS
        pnts3 <- SpatialPointsDataFrame(df_sbs.MO4_03[,c("declon","declat")],
                                        df_sbs.MO4_03,
                                        proj4string = crs(epsg4326nCRS2))
        ...
        df_sbs.MO4_03$declon5 <- df_sbs.MO4_03$declon + 0
        df_sbs.MO4_03$declat5 <- df_sbs.MO4_03$declat + 0.08
        
        #make points with decimal coordinates - this makes use of a different CRS
        pnts5 <- SpatialPointsDataFrame(df_sbs.MO4_03[,c("declon5","declat5")],
                                        df_sbs.MO4_03,
                                        proj4string = crs(epsg4326nCRS2))
        ...
        axis(2, at = yy, las=1, cex.axis=2.0) #las=1 turns the 
        #labels at the tick marks on the axis
        ...
        redlatpt1 <- (max(as.data.frame(pnts4)$declat)-min(as.data.frame(pnts4)$declat))/250.6
        #add text to the same lon lat position for harbour, or for month
        # text(as.data.frame(pnts)$declon,
        #      as.data.frame(pnts)$declat-redlatpt1,
        #      as.data.frame(pnts)$Harbour.abbr1,
        #      pos=1,
        #      cex= 1)
        # #add the the eDNA evaluations for sampling points 
        #as symbols coloured by column
        plot(pnts,add = TRUE,pch = 22,col = "black",
             bg = c("white"),cex = 4.0)
        #head(as.data.frame(pnts4),6)
        #add text to the same lon lat position for harbour, or for month
        text(as.data.frame(pnts)$declon,
             as.data.frame(pnts)$declat+0.10,
             as.data.frame(pnts)$nosiHp_em,
             col="black",
             pos=1,
             cex= 1.8)
  ...
  for (yr_smpl in yrs){
    ...
    for (season in rctse){
        ...
        #start curly bracket for 'else if' test testing whether the dimensions on the data frame is more than one, if it is then make the plot on the map
      {
        ...
        #add the the eDNA evaluations for sampling points 
        #as symbols coloured by column
        plot(pnts,add = TRUE,pch = 25,col = "black",
             bg = c("white"),cex = 4.0)
        #head(as.data.frame(pnts4),6)
        #add text to the same lon lat position for harbour, or for month
        text(as.data.frame(pnts)$declon,
